import {
  ...
  export class IndexedStore {
    ...
    open() {
      ...
      let request = this.idb.open(DB_NAME, 1)
    ...
    getKeys() {
      ...
      return this.getAllItems(CACHE_KEY_NAME)
        .then(values => {
          values.forEach((val) => this.keys.add(val.key))
